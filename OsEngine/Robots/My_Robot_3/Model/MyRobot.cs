...
namespace OsEngine.Robots.My_Robot.Model
{
    public class MyRobot : BotPanel
    {
        ...
        public MyRobot(string name, StartProgram startProgram) : base(name, startProgram)
        {
            ...
            paramStringMode = this.CreateParameter("Mode", "Edit", new[] { "Edit", "Trade" }); // создаём параметр для оптимизации
            
            paramIntLot  = this.CreateParameter("Lot", 1, 1, 100, 1); // создаём другой параметр для оптимизации
            paramIntTake = this.CreateParameter("Take", 1, 1, 100, 1);
            paramIntStop = this.CreateParameter("Stop", 1, 1, 100, 1);
        }
        ...
        public StrategyParameterString paramStringMode { get; set; }
